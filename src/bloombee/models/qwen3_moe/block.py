"""BloomBee block wrapper for Qwen3-MoE (e.g. Qwen3-30B-A3B).

The contract is identical to :mod:`bloombee.models.qwen3.block` so that the
server's pipeline-parallel backend, DHT allocation, and ``cache_compat``
machinery all stay untouched. Only the base class differs.
"""
...
try:
    from transformers.models.qwen3_moe.modeling_qwen3_moe import (
        Qwen3MoeDecoderLayer as _BaseDecoderLayer,
        Qwen3MoeRotaryEmbedding as _RotaryEmbedding,
    )
    from transformers.models.qwen3_moe import Qwen3MoeConfig as _BaseBlockConfig
except ImportError:  # pragma: no cover - older transformers fallback
    from transformers.models.qwen3.modeling_qwen3 import (
        Qwen3DecoderLayer as _BaseDecoderLayer,
        Qwen3RotaryEmbedding as _RotaryEmbedding,
    )
    from transformers.models.qwen3 import Qwen3Config as _BaseBlockConfig


class WrappedQwen3MoeBlock(_BaseDecoderLayer):
    ...
    def _apply(self, fn, recurse=True):
        ...
        # Same rationale as the dense Qwen3 wrapper: bare-block initialization
        # bypasses HF's _keep_in_fp32_modules machinery.
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            # Fix dtype and device mismatch (analogous to the dense Qwen3 path).
        ...
        # attention_mask=None. BloomBee wraps a bare Qwen3MoeDecoderLayer (no
        # Qwen3MoeModel in front to build the mask), so build the additive
        # causal mask here, exactly as the dense wrapper does.
        if attention_mask is None:
            ...
            # BloomBee's backend builds the mask as [B, S, K]; MoE attention
        ...
        # tf 5.x attention needs cache_position to know where to write new KV.
        ...
        # MoE layer returns plain Tensor; dense Qwen3 returns Tensor or tuple.
    ...
    def _reorder_cache_to_bloom(
        ...
        key_states, value_states = key_value
        # Use reshape (not view) since DynamicCache tensors may be non-contiguous
