...
- MiniMax M2.7/M3: M2.7 REAP is now explicitly a native BloomBee distributed-path target, not just GGUF. Exact config/native-contract scan is green at `minimax-m27-reap-native-contract-scan-20260706.json` (MiniMaxM2ForCausalLM/minimax_m2, 62 layers, 154 experts, top-k 8, MTP enabled, no exact sparse-attention flag), native `minimax_m2` wrapper/base-decoder MTP guard tests are green, and no route/demo promotion is allowed until real-weight one-block proof, full/MTP-safe proof, and load gates pass. GGUF/llama.cpp smoke is side evidence only; peer RAM is not additive there. M3 remains blocked by native wrapper/sparse-attention and huge memory.
