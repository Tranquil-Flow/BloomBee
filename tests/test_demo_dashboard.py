...
def test_dashboard_data_surfaces_devices_routes_benchmarks_and_evidence(tmp_path: Path):
    ...
    assert doc["mvp_status"]["post_mvp_task_summary"] == {"complete": 2, "partial": 5, "pending": 0, "blocked": 1, "total": 8}
    ...
    assert "Post-MVP backlog task summary: 2 complete, 5 partial, 0 pending, 1 blocked" in html
    ...
    assert "All-task summary: 11 complete, 5 partial, 0 pending, 1 blocked" in html
