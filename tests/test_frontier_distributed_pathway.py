...
def test_qwen36a_alias_reports_config_scan_needed_before_main_pathway_claims():
    from mvp_capabilities.frontier_distributed_pathway import build_frontier_distributed_pathway_report
    ...
    assert report["main_bloombee_pathway"]["usable_now"] is False
    assert report["main_bloombee_pathway"]["config_scan_required"] is True
    assert "exact_hf_config_not_scanned_in_repo" in report["main_bloombee_pathway"]["blocked_reasons"]
    assert report["external_runtime_pathway"]["can_pool_peer_memory_now"] is False
    assert report["external_runtime_pathway"]["single_host_required_free_gb"] == 80.0
    assert report["recommendation"]["next_engineering_lane"] == "config_scan_then_native_bloombee_adapter_or_external_rpc"
