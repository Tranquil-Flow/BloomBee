...
TARGETS: tuple[TargetSpec, ...] = (
    ...
    TargetSpec(
        ...
        bloombee_wrapper_name="qwen3_5_moe_text",
        config_scan_required=False,
        adapter_steps=(
            "exact config scan passed at distributed_evidence/qwen36a/qwen36a-config-scan-20260706.json",
            "map qwen3_5_moe/qwen3_5_moe_text to the native backend state-cache contract before runtime serving",
            "prove one-block server forward/backward or forward-only direct RPC for the exact Qwen36A row",
            "then prove multi-block split, full/cache generation, and load gates before route/demo promotion",
        ),
        default_next_lane="native_bloombee_adapter_before_route_promotion",
    ),
)
