...
PLANNED_TASKS: tuple[PlanTask, ...] = (
    ...
    PlanTask(
        ...
        status="partial",
        evidence="speculative_decode_plan.py defines verifier-authoritative draft-provider roles and phone-as-draft-only policy; draft_provider.py provides a deterministic provider interface and accepted/rejected exact-token counters for dashboard smoke reports; draft_provider_bridge.py exposes stdio JSONL transport for phone/Termux bridge tests; termux_draft_smoke.py verified a real Pixel 8 Pro Termux draft-contract smoke with proposed=3 accepted=2 rejected=1; no live repeated match-rate/latency proof yet",
        next_step="measure repeated phone bridge latency and exact-token acceptance rate against a verifier-only baseline before enabling speedup claims",
    ),
    PlanTask(
        id="phone_worker",
        label="Phone as useful inference or draft worker",
        status="partial",
        evidence="mobile capability fields exist in peer_scan.py; draft_provider.py defines the phone-compatible draft-provider contract; draft_provider_bridge.py provides stdio JSONL bridge groundwork; m4pro ADB pushed/typed a short command into Termux and verified real Pixel 8 Pro JSON evidence (Android SDK 36, Tensor G3, aarch64, Python 3.13.13) at mvp_capabilities/distributed_evidence/phone/termux-draft-smoke-20260704T095557Z.json; no real throughput, repeated bridge latency, or BloomBee block-serving proof yet",
        next_step="measure repeated phone draft latency/acceptance rate, then try one-block TinyLlama serving before counting phone compute",
    ),
    PlanTask(
        id="physical_showcase",
