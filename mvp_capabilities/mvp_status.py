...
PLANNED_TASKS: tuple[PlanTask, ...] = (
    ...
    PlanTask(
        ...
        status="partial",
        evidence="continuous_batching.py adds a pure round-robin decode scheduler simulation with late-arrival admission, padded batch inputs, and per-request deinterleaving; live_continuous_batching.py adds an opt-in injected-step live-loop unit; remote_generation.py and inference_session.py now expose an opt-in BLOOMBEE_ENABLE_LIVE_CONTINUOUS_BATCHING path that records tick rows and supports same-arrival batched greedy rows through the real client generation seam. Claim-bounded evidence remains at mvp_capabilities/distributed_evidence/post_mvp/continuous-batching-scheduler-20260704.json, mvp_capabilities/distributed_evidence/post_mvp/continuous-batching-live-adapter-20260705.json, mvp_capabilities/distributed_evidence/post_mvp/live-continuous-batching-loop-unit-20260705.json, and mvp_capabilities/distributed_evidence/post_mvp/continuous-batching-same-arrival-client-20260705.json; no live server late-arrival parity proof, wall-clock speedup, or demo promotion yet",
        next_step="use BLOOMBEE_ENABLE_LIVE_CONTINUOUS_BATCHING for a real live-server concurrent-arrival parity run, then measure wall-clock throughput before any demo or speedup promotion",
    ),
    PlanTask(
        id="kv_prefix_reuse",
