...
PLANNED_TASKS: tuple[PlanTask, ...] = (
    ...
    PlanTask(
        ...
        label="Qwen35B/Qwen36A native candidate branch",
        status="partial",
        evidence="Qwen/Qwen-AgentWorld-35B-A3B is memory-fit for synthetic 10-laptop planning and remains a main native BloomBee distributed path target, not a single-host GGUF fallback. Text-tower qwen3_5_moe_text wrapper/package now has import/config, full_attention block contract, linear_attention local state round-trip, and backend raw conv/recurrent descriptor/materialization/select/update tests green at mvp_capabilities/distributed_evidence/qwen35b/qwen-agentworld-35b-text-wrapper-gate-20260704.json; local one-block host preflight evidence at mvp_capabilities/distributed_evidence/qwen35b/qwen35b-oneblock-host-preflight-20260705T214226Z.json records this 16GB M4 host as blocked-by-host-memory against the 80GB requirement; there is still no one-block server proof and no demo/route promotion. User-requested Qwen36A/Qwen3.6 lane is explicitly tracked as Qwen/Qwen3.6-35B-A3B in mvp_capabilities/distributed_evidence/stretch/frontier-distributed-pathway-qwen36a-current-20260706.json; exact config scan evidence mvp_capabilities/distributed_evidence/qwen36a/qwen36a-config-scan-20260706.json records model_type=qwen3_5_moe, text model_type=qwen3_5_moe_text, 40 layers, hidden 2048, 30 linear_attention layers, 10 full_attention layers, 256 experts, and top-k 8; exact state-cache descriptor mapping evidence mvp_capabilities/distributed_evidence/qwen36a/qwen36a-state-cache-mapping-20260706.json maps the scanned text config to linear raw conv/recurrent descriptors and full-attention KV descriptors without live server proof; m4pro host preflight mvp_capabilities/distributed_evidence/qwen36a/qwen36a-oneblock-host-preflight-20260706.json records 48GB total/~33.2GB free against the 80GB requirement and is blocked-by-host-memory. It is desired on the same main native BloomBee distributed path, but remains blocked until exact one-block server proof proves runtime compatibility.",
        next_step="rerun Qwen3.5/Qwen36A one-block preflight on a host with at least 80GB free memory, then run one-block server proof, multi-block, full/cache generation, and load gates before showcase/safe-demo promotion",
    ),
    PlanTask(
        id="minimax_m3_candidate",
        label="MiniMax M2.7/M3 native frontier candidate",
        status="blocked",
        evidence="MiniMaxAI/MiniMax-M3 needs ~900GB-class runtime memory and lacks minimax_m3_vl/minimax_m3_vl_text + MiniMax Sparse Attention native BloomBee support. M2.7 REAP-139B-A10B is now catalogued as the nearer MiniMax frontier candidate (139B total/~10B active, MiniMaxM2ForCausalLM, 62 layers, hidden 3072, 154 experts, top-k 8, 196608 context) and is explicitly targeted for the main native BloomBee distributed path, not just single-host GGUF. native minimax_m2 wrapper contract exists and synthetic block tests cover KV prefill/decode, fp32 rotary, and config dispatch; current-memory evidence at mvp_capabilities/distributed_evidence/stretch/minimax-m27-reap-candidate-current-memory-20260706.json records local M4+m4pro as unable to fit i1-IQ2_XXS at that snapshot (m4pro had ~27.9GB free vs 36.8GB+margin), while the generated external llama.cpp/GGUF path remains a side smoke only after freeing enough memory or using a larger host. Family comparison evidence at mvp_capabilities/distributed_evidence/stretch/minimax-reap-family-comparison-current-20260706.json records M3 as likely stronger but not easier on current Macs (full M3 GGUF/JANG target ~128GB exceeds combined 64GB, vMLX absent), and records that M4+M4Pro memory is not additive for external runtimes; mvp_capabilities/distributed_evidence/stretch/frontier-distributed-pathway-minimax-m27-current-20260706.json records the desired main native BloomBee distributed path, blocked today by missing real-weight one-block proof and full MTP module proof. M2.7 memory watcher final evidence at mvp_capabilities/distributed_evidence/post_mvp/minimax-m27-reap-memory-watch-final-20260706T193704Z.json exited after 45/45 attempts with launch_ready=false, 33.163 GiB freeable, 40.8GB gate, and shortfall 7.637 GiB. M2.7 native contract scan at mvp_capabilities/distributed_evidence/post_mvp/minimax-m27-reap-native-contract-scan-20260706.json records exact config MiniMaxM2ForCausalLM/minimax_m2 with 62 layers, hidden 3072, 48 heads, 8 KV heads, 154 experts, top-k 8, use_mtp=true, all attn_type_list entries=1, no exact sparse-attention flag, required MTP module contract, and mtp_base_decoder_guard_available=true; it now records native_wrapper_package_present=true, wrapper_block_contract_available=true, and one_block_server_proven=false. M3 smoke plan remains at mvp_capabilities/distributed_evidence/stretch/minimax-m3-external-runtime-smoke-plan-20260705.json; route/demo status remains blocked.",
        next_step="run MiniMax-M2.7 REAP real-weight one-block server proof with explicit base_decoder_only MTP guard on suitable memory, then two-peer multi-block split, full/cache generation, and load gates; keep the M2.7 GGUF smoke as optional side diagnostics only, not the main plan",
    ),
    PlanTask(
