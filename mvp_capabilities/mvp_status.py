...
PLANNED_TASKS: tuple[PlanTask, ...] = (
    ...
    PlanTask(
        ...
        status="partial",
        evidence="speculative_decode_plan.py defines verifier-authoritative draft-provider roles and phone-as-draft-only policy; draft_provider.py provides a deterministic provider interface and accepted/rejected exact-token counters for dashboard smoke reports; draft_provider_bridge.py exposes stdio JSONL transport for phone/Termux bridge tests; termux_draft_smoke.py can render/verify pasteable Termux smoke evidence; no live match-rate/latency proof yet",
        next_step="capture real Termux smoke JSON, then run the same draft-provider contract over a phone HTTP/stdio bridge and measure live latency/exact-token acceptance rate before enabling speedup claims",
    ),
    PlanTask(
        id="phone_worker",
        label="Phone as useful inference or draft worker",
        status="partial",
        evidence="mobile capability fields exist in peer_scan.py; draft_provider.py defines the phone-compatible draft-provider contract; draft_provider_bridge.py provides stdio JSONL bridge groundwork; termux_draft_smoke.py provides a pasteable Termux smoke script/verifier; ADB daemon start is blocked in the Hermes sandbox, so real phone JSON must be pasted back or captured via an outside adb/SSH bridge; no real phone throughput, bridge latency, or BloomBee block-serving proof yet",
        next_step="paste/run the generated Termux smoke script on the phone, verify the captured JSON, then measure latency/acceptance rate and try one-block TinyLlama serving before counting phone compute",
    ),
    PlanTask(
        id="physical_showcase",
