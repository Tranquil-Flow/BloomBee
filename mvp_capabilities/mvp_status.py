...
MILESTONES: tuple[Milestone, ...] = (
    ...
    Milestone(
        ...
        completion=0.70,
        status="partial",
        evidence="demo_dashboard.py renders real peers, route cards, evidence, telemetry; progress bar not yet dashboard-wired",
        next_step="surface mvp_status.py output in dashboard",
    ),
    Milestone(
        id="join_flow",
        ...
        completion=0.35,
        status="partial",
        evidence="join_coordinator.py creates link offers and heartbeat rosters",
        next_step="add QR rendering and HTTP coordinator endpoints",
    ),
    Milestone(
        id="layer_planning",
        ...
        completion=0.45,
        status="partial",
        evidence="layer_planner.py emits deterministic contiguous ranges; server launch commands not yet wired",
        next_step="emit exact BloomBee server commands from assignments",
    ),
    Milestone(
    ...
    Milestone(
        ...
        completion=0.20,
        status="partial",
        evidence="Qwen3-8B and Qwen3-14B config-only prescan passed as supported qwen3 dense models",
        next_step=NEXT_GATE,
    ),
    Milestone(
