...
PLANNED_TASKS: tuple[PlanTask, ...] = (
    ...
    PlanTask(
        ...
        status="partial",
        evidence="kv_prefix_reuse.py adds a pure prefix-only reuse planner; kv_prefix_reuse_proof.py verifies same-prefix/varied-suffix artifacts with exact token parity, logit fingerprint parity, timing deltas, and fail-closed rejection on mismatches; bloombee.client.kv_prefix_reuse plus InferenceSession.record_kv_prefix_reuse_prefill now records same-prefix/varied-suffix prefill metadata behind BLOOMBEE_ENABLE_KV_PREFIX_REUSE, attaches it to first rpc_inference metadata, and the server observes it behind the same opt-in flag. Planner evidence remains at mvp_capabilities/distributed_evidence/post_mvp/kv-prefix-reuse-planner-20260704.json; live metadata evidence is at mvp_capabilities/distributed_evidence/post_mvp/kv-prefix-reuse-live-metadata-20260705.json; no live KV cache tensor reuse, runtime output parity proof, or speedup claim yet",
        next_step="use BLOOMBEE_ENABLE_KV_PREFIX_REUSE to capture live TinyLlama/Qwen3-8B prefill metadata plus baseline/reuse parity evidence, then implement actual server KV tensor reuse and measure memory/wall-clock impact before any demo promotion",
    ),
)
