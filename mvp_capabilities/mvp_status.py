...
PLANNED_TASKS: tuple[PlanTask, ...] = (
    ...
    PlanTask(
        ...
        status="blocked",
        evidence="MiniMaxAI/MiniMax-M3 needs ~900GB-class runtime memory and lacks minimax_m3_vl/minimax_m3_vl_text + MiniMax Sparse Attention native BloomBee support. A MiniMax-specific external-runtime smoke plan is generated at mvp_capabilities/distributed_evidence/stretch/minimax-m3-external-runtime-smoke-plan-20260705.json; this host is Apple M4/macOS with no nvidia-smi, so the external smoke cannot be executed here and route/demo status remains blocked.",
        next_step="run the generated vLLM/SGLang/TensorRT-LLM smoke only on approved NVIDIA hardware, save external_runtime_smoke evidence, then design LayerExecutor/native sparse-attention adapter only after that passes",
    ),
    PlanTask(
    ...
    PlanTask(
        ...
        status="partial",
        evidence="continuous_batching.py adds a pure round-robin decode scheduler simulation with late-arrival admission, padded batch inputs, and per-request deinterleaving; live_continuous_batching.py adds an opt-in injected-step live-loop unit; remote_generation.py and inference_session.py now expose an opt-in BLOOMBEE_ENABLE_LIVE_CONTINUOUS_BATCHING path that records tick rows and supports same-arrival batched greedy rows through the real client generation seam; continuous_batching_live_server_proof.py now verifies captured live-server late-arrival token/logit parity evidence fail-closed; continuous_batching_live_server_capture.py assembles baseline/continuous capture rows into verifier-ready evidence while refusing to infer server-observed status from client-only rows. Claim-bounded evidence remains at mvp_capabilities/distributed_evidence/post_mvp/continuous-batching-scheduler-20260704.json, mvp_capabilities/distributed_evidence/post_mvp/continuous-batching-live-adapter-20260705.json, mvp_capabilities/distributed_evidence/post_mvp/live-continuous-batching-loop-unit-20260705.json, mvp_capabilities/distributed_evidence/post_mvp/continuous-batching-same-arrival-client-20260705.json, mvp_capabilities/distributed_evidence/post_mvp/continuous-batching-live-server-proof-plan-20260705.json, and mvp_capabilities/distributed_evidence/post_mvp/continuous-batching-live-server-capture-harness-20260705T234958Z.json; no live server late-arrival parity proof, wall-clock speedup, or demo promotion yet",
        next_step="use BLOOMBEE_ENABLE_LIVE_CONTINUOUS_BATCHING to capture a real live-server concurrent/late-arrival artifact, verify it with continuous_batching_live_server_proof.py, then measure wall-clock throughput before any demo or speedup promotion",
    ),
    PlanTask(
        id="kv_prefix_reuse",
        label="Real prefill KV prefix reuse",
        status="partial",
        evidence="kv_prefix_reuse.py adds a pure prefix-only reuse planner; kv_prefix_reuse_proof.py verifies same-prefix/varied-suffix artifacts with exact token parity, logit fingerprint parity, timing deltas, fail-closed rejection on mismatches, and now emits a claim-bounded live capture plan; bloombee.client.kv_prefix_reuse plus InferenceSession.record_kv_prefix_reuse_prefill now records same-prefix/varied-suffix prefill metadata behind BLOOMBEE_ENABLE_KV_PREFIX_REUSE, attaches it to first rpc_inference metadata, and the server observes it behind the same opt-in flag. Planner evidence remains at mvp_capabilities/distributed_evidence/post_mvp/kv-prefix-reuse-planner-20260704.json; live metadata evidence is at mvp_capabilities/distributed_evidence/post_mvp/kv-prefix-reuse-live-metadata-20260705.json; live capture plan is at mvp_capabilities/distributed_evidence/post_mvp/kv-prefix-reuse-live-capture-plan-20260705.json; CLI exits non-zero on invalid evidence with regression artifact mvp_capabilities/distributed_evidence/post_mvp/kv-prefix-reuse-cli-fail-closed-20260705T225105Z.json; no live KV cache tensor reuse, runtime output parity proof, or speedup claim yet",
        next_step="use BLOOMBEE_ENABLE_KV_PREFIX_REUSE to capture live TinyLlama/Qwen3-8B prefill metadata plus baseline/reuse parity evidence, verify it with kv_prefix_reuse_proof.py, then implement actual server KV tensor reuse and measure memory/wall-clock impact before any demo promotion",
    ),
)
